...
class TestFaaasModels:
    ...
    def test_map_fusion_environment_family_from_dict_and_obj(self):
        ...
        assert fam_d.id == "fam1" and fam_d.display_name == "Family 1"
        assert fam_d.lifecycle_state == "ACTIVE"
        ...
        assert fam_o.id == "fam2" and fam_o.display_name == "Family 2"
        assert fam_o.lifecycle_state == "CREATING"
    ...
    def test_map_fusion_environment_includes_maintenance_policy_to_dict(
        ...
        assert env.id == "env1" and env.display_name == "Env 1"
        assert env.fusion_environment_type == "PRODUCTION"
        assert env.maintenance_policy == {"foo": 1}
        assert env.applied_patch_bundles == ["p1"]
        assert env.freeform_tags == {"a": "b"}
        assert env.defined_tags == {"ns": {"k": "v"}}
    ...
    def test_map_fusion_environment_status_id_fallback_and_details(self, monkeypatch):
        ...
        assert st.fusion_environment_id == "env1"
        ...
        assert st2.fusion_environment_id == "env2"
