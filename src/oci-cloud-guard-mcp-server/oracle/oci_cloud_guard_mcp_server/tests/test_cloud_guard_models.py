...
class TestProblemMapping:
    def test_map_problem_maps_all_known_fields_and_locks(self):
        ...
        assert mapped.id == "ocid1.problem.oc1..xyz"
        assert mapped.compartment_id == "ocid1.compartment.oc1..abc"
        assert mapped.detector_rule_id == "ocid1.rule.oc1..rule"
        assert mapped.region == "us-ashburn-1"
        assert mapped.regions == ["us-ashburn-1", "us-phoenix-1"]
        assert mapped.risk_level == "HIGH"
        assert mapped.risk_score == 85.5
        assert mapped.peak_risk_score_date == "2024-12-01T00:00:00Z"
        assert mapped.peak_risk_score == 90.1
        assert mapped.auto_resolve_date == "2025-01-01T00:00:00Z"
        assert mapped.peak_risk_score_lookup_period_in_days == 30
        assert mapped.resource_id == "ocid1.instance.oc1..res"
        assert mapped.resource_name == "my-instance"
        assert mapped.resource_type == "instance"
        assert mapped.labels == ["tag1", "tag2"]
        assert mapped.time_last_detected == now
        assert mapped.time_first_detected == pytest.approx(now - timedelta(days=1))
        assert mapped.lifecycle_state == "ACTIVE"
        assert mapped.lifecycle_detail == "OPEN"
        assert mapped.detector_id == "IAAS_ACTIVITY_DETECTOR"
        assert mapped.target_id == "ocid1.target.oc1..tgt"
        assert mapped.additional_details == {"k": "v"}
        assert mapped.description == "desc"
        assert mapped.recommendation == "reco"
        assert mapped.comment == "user comment"
        assert mapped.impacted_resource_id == "ocid1.impacted.oc1..res"
        assert mapped.impacted_resource_name == "impacted-name"
        assert mapped.impacted_resource_type == "compute"
        assert mapped.locks is not None
        assert len(mapped.locks) == 1
        assert isinstance(mapped.locks[0], ResourceLock)
        assert mapped.locks[0].type == "FULL"
