...
# @TODO Percentile should be converted to either common or custom percentages
...
def map_summarize_metrics_data_details(
    smd: oci.monitoring.models.SummarizeMetricsDataDetails,
) -> SummarizeMetricsDataDetails | None:
    """
    Convert an oci.monitoring.models.SummarizeMetricsDataDetails to
    oracle.oci_monitoring_mcp_server.models.SummarizeMetricsDataDetails.
    """
    if not smd:
        return None
    return SummarizeMetricsDataDetails(
        namespace=getattr(smd, "namespace", None),
        resource_group=getattr(smd, "resource_group", None),
        query=getattr(smd, "query", None),
        start_time=getattr(smd, "start_time", None),
        end_time=getattr(smd, "end_time", None),
        resolution=getattr(smd, "resolution", None),
    )
