...
def list_fusion_environment_families(
    ...
    while has_next_page:
        ...
        _append_items_from_response_data(raw_families, response.data)
        has_next_page = getattr(response, "has_next_page", False)
        next_page = getattr(response, "next_page", None)

    families = [map_fusion_environment_family(d) for d in raw_families]
...
def list_fusion_environments(
    ...
    while has_next_page:
        ...
        _append_items_from_response_data(raw_envs, response.data)
        has_next_page = getattr(response, "has_next_page", False)
        next_page = getattr(response, "next_page", None)

    environments = [map_fusion_environment(d) for d in raw_envs]
