...
class TestLoggingModels:
    ...
    def test_map_log_group_summary_none_and_values(self):
        ...
        assert dst.id == "lg1"
        assert dst.compartment_id == "comp1"
        assert dst.lifecycle_state == "ACTIVE"
    ...
    def test_map_log_summary_nested(self):
        ...
        assert mapped.id == "log1"
        assert mapped.configuration is not None
        assert mapped.configuration.archiving.is_enabled is True
    ...
    def test_map_log_nested(self):
        ...
        assert mapped.log_type == "CUSTOM"
        assert mapped.configuration.source.source_type == "OCISERVICE"
    ...
    def test_map_search_response_full(self):
        ...
        assert mapped.fields[0].field_name == "x"
        ...
        assert mapped.summary.result_count == 1

    def test_map_helpers_individual(self):
        fi = map_field_info(SimpleNamespace(field_name="f", field_type="NUMBER"))
        assert isinstance(fi, FieldInfo)
        assert fi.field_type == "NUMBER"

        sr = map_search_result(SimpleNamespace(data={"a": 1}))
        assert isinstance(sr, SearchResult)
        assert sr.data == {"a": 1}

        srs = map_search_result_summary(SimpleNamespace(result_count=3, field_count=2))
        assert isinstance(srs, SearchResultSummary)
        assert srs.field_count == 2
