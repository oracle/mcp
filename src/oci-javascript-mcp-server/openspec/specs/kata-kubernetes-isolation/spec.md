# Kata Kubernetes Isolation Specification
...
Each standard and Kata in-cluster example bundle SHALL deploy a trusted host in a namespace separate from the execution namespace, using the trusted host service account, its exact Kubernetes profile, and downward-API values for its namespace, pod name, and pod UID. A Kata host SHALL additionally configure the exact reviewed RuntimeClass and handler. Each bundle SHALL deploy the independent cleanup-only reconciler under its distinct identity. The trusted host alone MAY mount the OCI credential Secret; the execution runner and cleanup reconciler SHALL receive no OCI credential mount. The host and runner images SHALL be immutable digest references, and the runner digest configured for the host SHALL be the exact digest required by that profile's admission policy. Example assets SHALL not contain credential material.

#### Scenario: Standard trusted-host topology
- **WHEN** an operator applies the standard in-cluster example bundle after supplying reviewed image digests and a host-namespace OCI credential Secret
- **THEN** it SHALL create the trusted host Deployment with only its required in-cluster profile, runner, OCI configuration, and downward-API identity settings
- **AND** the host Secret SHALL be mounted read-only only by that Deployment
- **AND** the runner and cleanup reconciler SHALL not receive OCI configuration or credential mounts

#### Scenario: Runner image is admission-aligned
- **WHEN** the standard trusted host is configured with its runner image digest
- **THEN** the standard admission policy SHALL require exactly that same runner image digest for execution pods
- **AND** a changed host runner digest without a matching admission-policy update SHALL fail manifest-contract validation

#### Scenario: Kata trusted-host topology
- **WHEN** an operator applies the Kata in-cluster example bundle after supplying reviewed image digests, an exact reviewed RuntimeClass/handler, and a host-namespace OCI credential Secret
- **THEN** it SHALL create the trusted host Deployment with the `kata-in-cluster` profile, the exact RuntimeClass and handler, runner image, OCI configuration, and downward-API identity settings
