...
@mcp.tool(description="List compartments in a given tenancy")
...
def list_availability_domains(
    ...
    limit: Optional[int] = Field(
        None,
        description="The maximum amount of availability domains to return. If None, there is no limit.",
        ge=1,
    ),
...
def create_auth_token(user_id: str) -> AuthToken:
    try:
        ...
        response: oci.response.Response = client.create_auth_token(user_id)
