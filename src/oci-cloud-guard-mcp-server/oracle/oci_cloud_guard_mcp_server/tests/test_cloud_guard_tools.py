...
class TestResourceSearchTools:
    ...
    @pytest.mark.asyncio
    @patch("oracle.oci_cloud_guard_mcp_server.server.get_cloud_guard_client")
    async def test_list_all_problems_with_filters(self, mock_get_client):
        mock_client = MagicMock()
        mock_get_client.return_value = mock_client
        ...
        async with Client(server.mcp) as client:
            await client.call_tool(
                "list_problems",
                {
                    ...
                    "risk_level": "HIGH",
                    "detector_rule_ids": ["rule1"],
                },
            )

        mock_client.list_problems.assert_called_once()
        kwargs = mock_client.list_problems.call_args.kwargs
        assert kwargs["risk_level"] == "HIGH"
        assert kwargs["detector_rule_id_list"] == ["rule1"]
...
class TestServer:
    @patch("oracle.oci_cloud_guard_mcp_server.server.oci.auth.signers.TokenExchangeSigner", return_value="signer")
    @patch("oracle.oci_cloud_guard_mcp_server.server.get_access_token")
    ...
    @patch("oracle.oci_cloud_guard_mcp_server.server.get_access_token", return_value=None)
    ...
    @patch("oracle.oci_cloud_guard_mcp_server.server._get_http_config_and_signer")
    def test_get_cloud_guard_client_http(self, mock_http_config_and_signer, mock_client):
    ...
    @patch("oracle.oci_cloud_guard_mcp_server.server.OCIProvider")
